...
namespace KursOragnisation
{
	public partial class OfferForm : Form
	{
		...
		private void OfferForm_Load(object sender, EventArgs e)
		{
			if (isUpdating)
			{
				SqlCommand getRow = new SqlCommand("SELECT (SELECT CONCAT((SELECT m.name FROM Manufacturer m WHERE d.manufacturer_id = m.id), ' ', d.name, ' ', copacity) FROM Disk d WHERE o.disk_id = d.id) as disk, shop, url, price FROM Offer o WHERE id = @id", connection);
